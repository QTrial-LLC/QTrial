...
Each attempt at submitting results to AKC electronically. For MVP (Obedience and Rally), the electronic submission consists of two PDFs attached to an email: the marked catalog and the populated AKC Report of Rally Trial (Form JOVRY8) or Obedience equivalent. Per Deborah's 2026-04-23 correction, the judges books that AKC receives are the PHYSICAL originals with the judge's wet signature, mailed separately; they are not part of the electronic submission and are NOT tracked here. XML submission is Agility-only and deferred post-MVP.
...
| `club_id` | UUID | tenant root |
| `trial_id` | UUID | FK |
| `submission_type` | ENUM | `pdf_package` (MVP: marked catalog + form JOVRY8), `xml` (post-MVP Agility), `csv` (fallback) |
...
Per-class judges book PDF artifacts (pre-trial blanks for printing) live on `trial_class_offerings`, not on `submission_records`. `submission_records` tracks the electronic submission event only: the marked catalog PDF and the populated AKC form, as attached to the email to AKC. Handling of post-trial signed-scan storage is an open question for PR 2c scoping.
